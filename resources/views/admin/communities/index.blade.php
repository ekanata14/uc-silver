...
@section('content')
    <div class="mb-4 flex justify-between items-center">
        <h1 class="text-2xl font-bold text-primary-lighter">Community</h1>
    ...
    <div class="bg-dark-light rounded-lg p-6 border border-primary/20 jewelry-glow">
        ...
        <div class="relative w-full overflow-auto">
            ...
            <thead class="[&_tr]:border-b">
                ...
                <th class="h-12 px-4 text-left align-middle font-medium text-primary-lighter">Image</th>
            ...
            <tbody class="[&_tr:last-child]:border-0">
                ...
                <tr class="border-primary/10 hover:bg-primary/10">
                    ...
                    <td class="p-4 align-middle font-medium text-gray-200">{{ $community->name }}</td>
                    <td class="p-4 align-middle text-gray-300">{{ $community->description }}</td>
                    <td class="p-4 align-middle">
                    @if($community->image)
                        <img src="{{ asset('storage/' . $community->image) }}" alt="{{ $community->name }}" class="h-10 w-10 object-cover rounded" />
                    @else
                    ...
                    <form action="{{ route('admin.communities.destroy', $community->id) }}" method="POST"
                        class="inline delete-community-form">
                        @csrf
                        @method('DELETE')
                        <button type="button"
                ...
                @empty
                <tr>
                    <td colspan="5" class="p-4 text-center text-gray-400">No communities found.</td>
                </tr>
                @endforelse
            </tbody>
            </table>
